...
get_nearbystations<-function(start,end){
...
google_route<-function(start,end){
...
mystations<-get_nearbystations(start,end)
mygooglepoints<-google_route(start,end)

#plot the fueling stations on the map
leaflet() %>%
  addTiles() %>%
  addMarkers(data = mystations, lat = ~Latitude, lng = ~Longitude,icon =fuelstationicon) %>%
  addPolylines(data = mygooglepoints,lat = ~lat, lng = ~lon)
